...
#include <iostream>
#include <Windows.h> //Включает основные функции и структуры Windows API.
#include <WinSock2.h> //Подключает функции и структуры для работы с сокетами в Windows.
#include <WS2tcpip.h> //Включает дополнительные функции для работы с сокетами, такие как getaddrinfo.
using namespace std;
...
int main()
{
    ...
    const char* sendBuffer1 = "Privet! (client)";
    const char* sendBuffer2 = "Kak dela? (client)"; //Строки, которые будут отправлены серверу.
